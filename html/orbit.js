...
//
//	Set up the scene, the renderer, the camera, and the orbit controls
//	Then the lights
//	Then the actual objects

function init() {
	var container;
	var light, light_theta, light_rad, light_x, light_y, light_z;
	...
	light = new THREE.PointLight(0xffffff, 3, 0, 2);
	light_theta = theta;
	light_rad = 600;
	...
	light_theta = theta + Math.PI * 2.0 / 3.0;
	light_x = light_rad * Math.sin(light_theta);
	light_y = light_rad * Math.cos(light_theta);
	light_z = 800;
	light.position.set(light_x, light_y, light_z);
	...
	line_material = new THREE.LineBasicMaterial({color:0x333333, linewidth:2});
	add_state(utah, 160, 0x00ff00);
	add_state(arizona, 110, 0xcccc00);
	add_state(new_mexico, 55, 0xff0000);
}
...
function add_state(points, height, color) {
